  ...
  return new Observable({
    ...
    define: {
      ...
      complete () {
        ...
        this.timeout = setTimeout(this.pollStatus.bind(this), 3000)
      },
      pollStatus () {
        this.clearTimeout()
        ...
        now.get(`deployments/${this.id.compute()}`, token, false)
          .on('data', (deployment) => {
            if (deployment.state === 'READY') {
              return this.emit('ready')
            }

            this.timeout = setTimeout(this.pollStatus.bind(this), this.tryCount * 3000)
          })
      ...
      load (deployment) {
        this.setAlias()
        this.now.findDeployment(deployment)
          .then(found => {
            this.set({
              url: found.url,
              id: found.uid
